...
if (document.getElementById("PhoneLogin")){
...
//Toggling Register and login form
if (document.getElementsByClassName("login-area")[0] != null) {
    
    document.getElementById("linkRegister").addEventListener('click',(e)=>{
        e.preventDefault();
        document.querySelectorAll(".toggling-block").forEach(element=>{
            element.style.display = "none";
        })

        document.querySelector(".backtoLogin").style.display= "block";
        document.querySelector(".login-area .header .headText").remove();

        let header = document.createElement("h5");
        header.classList = "text-center headText mt-2";
        header.innerHTML = "Finish signing up";

        document.querySelector(".login-area .header").appendChild(header);

        document.querySelector(".login-area .container").innerHTML = `<form id="registerForm">
    ...
    <div class="form-check">
      ...
      <label class="form-check-label" id="check-label" for="flexCheckDefault">I dont want to receive marketing messages from Airbnb.</label>
        ...
        //Handling back operation from register
        document.querySelector(".login-area .header .backtoLogin").addEventListener('click', (e) => {
            e.preventDefault();
            document.querySelector(".login-area .header .headText").remove();
            
            let header = document.createElement("h5");
            header.classList = "text-center headText mt-2";
            header.innerHTML = "Log in or sign up to book";

            document.querySelector(".login-area .header").appendChild(header);

            document.querySelector(".login-area .container").innerHTML = ` <form id="loginSignupForm" class="mt-4">
            <div class="form-floating">
            ...
            <label for="floatingSelect" class="float-labelform">Country/Region</label>
            </div>
            <div class="form-floating mb-3">
            <input type="text" class="form-control" id="floatingInput" placeholder="name@example.com" name="phoneNumber">
            <label for="floatingInput" class="float-labelform">Phone Number</label>
            </div>
            <div class="form-floating mb-3" style="display:none">
            <input type="email" class="form-control" id="floatingInput" placeholder="name@example.com" name="email">
            <label for="floatingInput" class="float-labelform">Email</label>
            </div>
            <div class="form-floating mb-3">
                <input type="password" class="form-control" id="floatingInput" placeholder="name@example.com" name="password">
                <label for="floatingInput">Password</label>
            ...
            <div class="mb-4">
            <button class="btn" id="authSubmit" onclick="invokeMethod()">Continue</button>
            </div>
        </form>`;
        ...
        document.querySelector(".login-area .header .backtoLogin").style.display = "none";
        });
...
function submitBooking(){
    document.getElementById("finalLabel").appendChild(successMessage("Yay!,you successfully booked the property"));
}
...
function loginUser(){
    ...
    fetchResult.then(res => res.json()).then((response) => {
        if (response.errors){
            document.getElementById("finalLabel").appendChild(
